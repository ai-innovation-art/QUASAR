...
11. research - Find documentation, search for solutions, research libraries or APIs (KEYWORDS: how to, search, find docs, library, API, documentation)
...
- If query contains "search", "find documentation for", "lookup", "how to use" → classify as research
...
═══════════════════════════════════════════════════════════════
CORE PRINCIPLE: COMMUNICATE, THEN EXECUTE
═══════════════════════════════════════════════════════════════
...
- Use list_tree_fast() to quickly see the whole project structure (up to 3 levels deep)
- Use grep_search(query) for near-instant text search across the workspace
...
When you need information beyond the local workspace:
1. Use search_web(query) to find relevant URLs (Tavily/SearXNG).
2. Use read_url(url) to extract clean Markdown content from a specific site.
3. Use browse_interactive(action, url, ...) if you need to interact with a page 
   (clicking buttons, handling dynamic content, etc.).
4. Vision Support: Use browse_interactive(action='capture') to "see" a page when 
   DOM extraction isn't enough to understand the layout.
...
def _get_progress_message(tool_name: str, tool_args: dict) -> str:
    ...
    messages = {
        ...
        'grep_search': f"Searching workspace for \"{tool_args.get('query', '')}\"...",
        'list_tree_fast': f"Scanning directory tree at `{path or '.'}`...",
        'search_web': f"Searching the web for \"{tool_args.get('query', '')}\"...",
        'read_url': f"Extracting content from `{tool_args.get('url', '')}`...",
        'browse_interactive': f"Performing browser action: {tool_args.get('action', '')}...",
...
Ensure the refactored code is cleaner and more maintainable.""",
            ...
            TaskType.RESEARCH: """
Find relevant information, documentation, or code examples.
Search the workspace first for local context.
Search the web for external knowledge or latest updates.
Synthesize findings into a helpful answer.
